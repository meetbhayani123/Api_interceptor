      ...
      try {
        ...
        if (useProxy) {
          ...
          fetchOptions = {
            ...
            headers: {
              'content-type': (options.headers as Record<string, string>)?.['content-type'] || 'application/x-www-form-urlencoded',
            },
            body: options.body,
          };
          ...
          fetchOptions = {
            ...options,
            headers: { ...this.buildFreshHeaders(), ...(options.headers as Record<string, string> || {}) },
          };
        ...
        if (response.status === 403 || response.status === 429) {
          ...
          if (contentType.includes('text/html')) {
            ...
            lastError = new Error(
              `WAF blocked (${response.status}${isCloudflare ? ', Cloudflare' : ''}). ` +
